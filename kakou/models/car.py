...
class CarInfo(db.Model):
    ...
    date_upload = db.Column(db.DateTime, default=arrow.now().datetime)
    ...
    def __init__(self, date_upload=None, date_created=None, hphm='', wc='',
                 img_path='', banned=0):
        if date_upload:
            self.date_upload = date_upload
        else:
            self.date_upload = arrow.now().datetime
